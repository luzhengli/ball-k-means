                 ...
                 VectorOur &delta, MatrixOur &centers_dis)
{
    ...
    */
    ...
    if (iteration_counter == 1)
        centers_dis = cal_dist(new_centroids, new_centroids).array().sqrt();
    else
    {
        for (unsigned int i = 0; i < k; i++)
        {
                ...
                ClusterIndexVector &clusters_neighbors_index, ClusterDistVector &temp_dis)
{
    ...
    *     point_center_dist: distance from point in cluster to center.一个二维向量 每个一维向量元素表示一个簇中所有点到簇中心的距离* 
...
int main(int argc, char *argv[])
{
    ...
    cout << "start to fit..\n";
    VectorXi labels = ball_k_means(dataset, 50, false, true);
    cout << "fit over!\n";
